...
# ───────────────────────────────────────────────────────────────
# PIPELINE ENSEMBLE — 5 dedicated models, one per class
# ───────────────────────────────────────────────────────────────
...
# ───────────────────────────────────────────────────────────────
def run_inference(img_pil, conf=0.25):
    ...
    for cls_name, mdl in _ensemble.items():
        ...
        for box in results.boxes:
            ...
            # Use model class name (overrides whatever the model internally calls it)
            name = cls_name
...
_active_model_label = f"{len(_ensemble)}/5 models"
st.markdown(
    f"""
<div class="lv-header">
...
with tab_settings:
    ...
    with col_s1:
        ...
        active_label = f"{len(_ensemble)}/5 models loaded"
        st.markdown(
            f'''
<div class="card-row"><span class="text-muted">Models โหลดแล้ว</span><span class="text-mono text-blue">{len(_ensemble)}/5</span></div>
